...
fn check(map: &Vec<Vec<u8>>, pos: (i32, i32, u8), trails: &mut HashSet<(i32, i32)>) {
    for pattern in [(1, 0), (0, 1), (-1, 0), (0, -1)] {
        ...
        if v == pos.2+1 {
            if v == 9 {
                trails.insert(next_pos);
            }
            else {
                check(map, (next_pos.0, next_pos.1, v), trails);
            }
        }
    }
}