...
fn main() {
    ...
    for line in input.lines() {
        ...
        connections.entry(l.to_string()).or_insert_with(Vec::new).push(r.to_string());
        connections.entry(r.to_string()).or_insert_with(Vec::new).push(l.to_string());
    }

    let mut result = 0;
    let mut checked: HashSet<Vec<String>> = HashSet::new();
    ...
    for (host_1, other_hosts) in connections.iter() {
        for host_2 in other_hosts.iter() {
            ...
            for host_3 in other_hosts.iter() {
                ...
                let mut hosts = vec![host_1.clone(), host_2.clone(), host_3.clone()];
